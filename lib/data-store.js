...
class DataStore {
  ...
  createAttendanceLog(logData) {
    const newLog = {
      ...
      displayInTime: logData.displayInTime || logData.inTime,
      outTime: null,
      displayOutTime: null,
      departmentId: logData.departmentId,
      workingCategory: logData.workingCategory || logData.departmentId,
      supervisorId: logData.supervisorId,
      outSupervisorId: null,
      totalHours: 0,
      status: "IN",
    }
  ...
  // Enhanced reporting with supervisor and category tracking
  getEmployeeAttendanceSummary(employeeId, startDate, endDate) {
    const logs = this.attendanceLogs.filter((log) => {
      const logDate = new Date(log.date)
      const start = new Date(startDate)
      const end = new Date(endDate)
      return log.employeeId === employeeId && logDate >= start && logDate <= end && log.outTime
    })
    ...
    logs.forEach((log) => {
      const category = log.workingCategory || log.departmentId
      const supervisor = log.supervisorId

      // Category stats
      if (!categoryStats[category]) {
        categoryStats[category] = { days: 0, hours: 0, name: this.getDepartmentById(category)?.name || category }
      }
      categoryStats[category].days += 1
      categoryStats[category].hours += log.totalHours

      // Supervisor stats
      if (supervisor) {
        if (!supervisorStats[supervisor]) {
          supervisorStats[supervisor] = { days: 0, hours: 0, name: this.getUserById(supervisor)?.name || supervisor }
        }
        supervisorStats[supervisor].days += 1
        supervisorStats[supervisor].hours += log.totalHours
      }
    ...
    return {
      ...
      logs: logs.map((log) => ({
        ...log,
        employee: this.getEmployeeById(log.employeeId),
        department: this.getDepartmentById(log.departmentId),
        supervisor: this.getUserById(log.supervisorId),
        outSupervisor: this.getUserById(log.outSupervisorId),
      })),
    }
  }
  ...
  getMonthlyAttendanceReport(month, year) {
    ...
    return logs.map((log) => {
      ...
      const supervisor = this.getUserById(log.supervisorId)
